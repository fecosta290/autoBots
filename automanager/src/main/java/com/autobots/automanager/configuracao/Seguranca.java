...
public class Seguranca extends WebSecurityConfigurerAdapter {
    ...
    private static final String[] rotasPublicas = { "/", "/usuario/{id}", "/usuarios", "/empresas", "/empresa/{id}", "/mercadorias", "/mercadoria/{id}", "/veiculos" };

    @Override
    protected void configure(HttpSecurity http) throws Exception {
        http.cors().and().csrf().disable();

        http.authorizeRequests().antMatchers(rotasPublicas).permitAll().anyRequest().authenticated();
